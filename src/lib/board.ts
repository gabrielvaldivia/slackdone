...
export interface UnifiedBoardResult {
  columns: BoardColumn[];
  lists: {
    listId: string;
    listTitle: string;
...
export async function fetchUnifiedBoard(userId: string): Promise<UnifiedBoardResult> {
  ...
  for (const key of orderedKeys) {
    ...
    if (col) {
      mergedColumns.push({ id: key, name: col.name, items: col.items });
    }
  }

  // Hydrate completedAt timestamps from Firestore
  const completedAtMap = await getCompletedAtMap(userId);
